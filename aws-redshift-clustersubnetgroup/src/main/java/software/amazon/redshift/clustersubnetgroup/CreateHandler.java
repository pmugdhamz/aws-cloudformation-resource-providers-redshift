...
public class CreateHandler extends BaseHandlerStd {
    ...
    protected ProgressEvent<ResourceModel, CallbackContext> handleRequest(
        ...
        // Combine all tags (resource + stack + system)
        Map<String, String> allTags = Maps.newHashMap();

        // Add resource level tags
        Map<String, String> resourceTags = Translator.translateFromResourceModelToSdkTags(model.getTags());
        allTags.putAll(Optional.ofNullable(resourceTags).orElse(Collections.emptyMap()));

        // Add stack level tags
        allTags.putAll(Optional.ofNullable(request.getDesiredResourceTags())
                .orElse(Collections.emptyMap()));

        // Add system tags
        allTags.putAll(Optional.ofNullable(request.getSystemTags())
                .orElse(Collections.emptyMap()));

        return ProgressEvent.progress(model, callbackContext)
                .then(progress -> proxy.initiate(String.format("%s::Create", CALL_GRAPH_TYPE_NAME),
                                proxyClient, progress.getResourceModel(), progress.getCallbackContext())
                        .translateToServiceRequest(rm -> Translator.translateToCreateRequest(
                                rm.getClusterSubnetGroupName(), rm, allTags))
